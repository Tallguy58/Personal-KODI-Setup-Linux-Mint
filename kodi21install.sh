...
apt-get -y -qq update -oAcquire::AllowInsecureRepositories=true --allow-unauthenticated >/dev/null

apt-get -y -qq install debian-keyring --allow-unauthenticated >/dev/null
apt-get -y -qq install debian-archive-keyring --allow-unauthenticated >/dev/null

## ADD KEYRINGS
wget -q -O /tmp/deb-multimedia-keyring_2016.8.1_all.deb https://www.deb-multimedia.org/pool/main/d/deb-multimedia-keyring/deb-multimedia-keyring_2016.8.1_all.deb >/dev/null
dpkg -i /tmp/deb-multimedia-keyring_2016.8.1_all.deb >/dev/null
rm -f /tmp/deb-multimedia-keyring_2016.8.1_all.deb

## DEBIAN.ORG KEYRING URL - https://packages.debian.org/sid/all/debian-keyring/download
wget -q -O /tmp/debian-keyring_2024.03.24_all.deb http://ftp.wa.au.debian.org/debian/pool/main/d/debian-keyring/debian-keyring_2024.03.24_all.deb >/dev/null
dpkg -i /tmp/debian-keyring_2024.03.24_all.deb >/dev/null
rm -f /tmp/debian-keyring_2024.03.24_all.deb

apt-get -y -qq update -oAcquire::AllowInsecureRepositories=true --allow-unauthenticated >/dev/null

##UPDATE KODI
apt-get -y -qq install --allow-unauthenticated --reinstall libgcrypt20 liblcms2-2 libwebp7 libzstd1 libcodec2-1.0 libhwy1 libjxl0.7 libopencore-amrnb0 libopencore-amrwb0 libsnappy1v5 libvo-amrwbenc0 libvpx7 libwebpmux3 libcjson1 libmbedcrypto7 libsrt1.5-gnutls libfftw3-double3 libjpeg62-turbo liblerc4 libtiff6 liblept5 libtesseract5 -t stable >/dev/null
apt-get -y -qq install --allow-unauthenticated --reinstall libass9 libavcodec60 libavfilter9 libavformat60 libavutil58 libbluray2 libcdio19 libcec6 libcrossguid0 libiso9660-11 libpostproc57 libswresample4 libswscale7 libudfread0 -t stable >/dev/null
apt-get -y -qq install --allow-unauthenticated kodi -t bookworm-backports >/dev/null

## REMOVE FROM KEYRINGS, PREFERENCES AND SOURCES FROM REGISTRY TO AVOID PROBLEMS??
...
apt-get -y -qq install --fix-broken >/dev/null
...
apt-get -y -qq update --fix-missing >/dev/null
apt-get -y -qq update >/dev/null
apt-get -y -qq autoremove >/dev/null
